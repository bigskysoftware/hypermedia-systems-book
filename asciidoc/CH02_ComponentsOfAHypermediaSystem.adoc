...
`POST`::
  A POST request submits data to the specified resource. This can often result in a mutation of state on the server (e.g.
  creation or modification of a resource) but the exact semantics are determined by the server.

`PUT`::
  A PUT request requests that the server completely replace the data of the specified resource with the given data. This often results in a mutation of state on the server if the request is accepted.

`PATCH`::
  A PATCH request requests a partial modification of the specified resource. This often results in a mutation of state on the server if the request is accepted.

`DELETE`::
  A DELETE request deletes the specified resource. This often results in a mutation of state on the server if the request is accepted.
